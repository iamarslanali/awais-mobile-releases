# Awais Mobile - releases

Installers only. No source code lives here.

This repository is the auto-update feed for Awais Mobile. The app reads only `awais-mobile.yml` from the newest published release.

## Rules

- Every release needs all three files: `awais-mobile.yml`, the installer `.exe`, and the `.exe.blockmap`. A missing `.yml` means the shop never updates, and the release still looks complete.
